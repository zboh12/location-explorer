...
export const jDBSCAN = function () {
  ...
  function haversine_distance (point1, point2) {
    ...
    let precision = precision ? precision : 4

    let R = 6371
    let lat1 = point1.location.latitude * Math.PI / 180,
      lon1 = point1.location.longitude * Math.PI / 180
    let lat2 = point2.location.latitude * Math.PI / 180,
  ...
  function expand_cluster (point_idx, neighbours, cluster_idx) {
    ...
    for (let i = 0; i < neighbours.length; i++) {
      let curr_point_idx = neighbours[i]
  ...
  function expand_cluster_no_recur (point_idx, neighbours, cluster_idx) {
    clusters[cluster_idx - 1].push(point_idx) // add point to cluster
    status[point_idx] = cluster_idx	// assign cluster id
    while (neighbours.length > 0) {
      let curr_point_idx = neighbours.pop()
      if (status[curr_point_idx] === undefined) {
        status[curr_point_idx] = 0 // visited and marked as noise by default
        let curr_neighbours = get_region_neighbours(curr_point_idx)
        let curr_num_neighbours = curr_neighbours.length
        if (curr_num_neighbours >= minPts) {
          neighbours.push(...curr_neighbours)
        }
      }
      if (status[curr_point_idx] < 1) { // not assigned to a cluster but visited (= 0)
        status[curr_point_idx] = cluster_idx
  ...
  let dbscan = function () {
    ...
    for (let i = 0; i < data.length; i++) {
      if (status[i] === undefined) {
        ...
        let neighbours = get_region_neighbours(i)
        let num_neighbours = neighbours.length
        if (num_neighbours < minPts) {
          status[i] = 0 // noise
        } else {
          clusters.push([]) // empty new cluster
          let cluster_idx = clusters.length
          expand_cluster_no_recur(i, neighbours, cluster_idx)
        }
      }
    }
